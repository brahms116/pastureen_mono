...
fn open_menu_svg() -> Markup {
    html! {
        svg.pt-navbar-mobile-menu-button__icon x-show="!open" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" {
...
pub fn navbar(props: NavbarProps) -> Markup {
    html! {
        div x-data="{open: false}" {
            div.pt-navbar {
                a.pt-navbar-logo href=(props.logo_link) {
                    img.pt-navbar-logo__logo src=(props.logo_src) alt="logo" {}
                    h3.pt-navbar-logo__text { (props.logo_text) }
                }
                .pt-navbar-mobile-menu-button x-on:click = "open = !open; document.body.style.overflowY = 'hidden'" {
                    (open_menu_svg())
                }
                div.pt-navbar-menu {
                    @for item in props.nav_items {
                        a.pt-navbar-menu__item.pt-navbar-menu__item--active[item.is_active] href=(item.link) {
                            h5 { (item.text) }
                        }
                    }
                }
            }
            div.mobile-menu x-bind:style="open ? 'transform: translateX(-100%);' : 'transform: translateX(0%);'" {
                div.pt-navbar {
                    a.pt-navbar-logo href=(props.logo_link) {
                        img.pt-navbar-logo__logo src=(props.logo_src) alt="logo" {}
                        h3.pt-navbar-logo__text { (props.logo_text) }
                    }
                    .pt-navbar-mobile-menu-button x-on:click = "open = !open; document.body.style.overflowY = 'auto'" {
                        (close_menu_svg())
                    }
                }
                div.content-wrapper {
                            ...
                            div.mobile-menu-nav {
                                ...
                                div.mobile-menu-nav__list {
                                    @for (i, item) in props.nav_items.iter().enumerate() {
                                        a.mobile-menu-item.mobile-menu-item--active[item.is_active] href=(item.link) {
                                            h5 { (item.text) }
                                        }
